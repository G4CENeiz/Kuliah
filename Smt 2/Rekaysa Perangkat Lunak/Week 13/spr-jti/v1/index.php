...
$fileExists = file_exists('data.txt');
    if(!$fileExists)
        file_put_contents('data.txt', serialize([]));
?>

<div>
    <div class="w3-container w3-teal">
        <div class="w3-cell"><img width="60" height="60" src="logo-polinema.png" class="w3-image"/></div>
    ...
    if (isset($_POST['submit'])) {
        $entry = [
            ...
            'ruangan_yang_dipinjam' => $_POST['ruangan_yang_dipinjam'],
            'lantai'                => $_POST['lantai'],
        ];
        $fileExists = file_exists('data.txt');
        if(!$fileExists)
            file_put_contents('data.txt', serialize([$entry]));
        else {
            $fileContents = file_get_contents('data.txt');
            $existingData = empty($fileContents) ? [] : unserialize($fileContents);
            $existingData[] = $entry;
            file_put_contents('data.txt', serialize($existingData));
        }
    }
    $fileContents = file_get_contents('data.txt');
    $existingData = empty($fileContents) ? [] : unserialize($fileContents);
